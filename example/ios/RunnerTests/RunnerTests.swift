...
class IAMOverlayWindowTests: XCTestCase {

  func testPassthroughView_toastMode_passesThroughOnEmptyArea() {
    let container = IAMPassthroughView(frame: CGRect(x: 0, y: 0, width: 390, height: 844))
    container.isPassthroughEnabled = true

    // Direct hit on empty container returns nil
    let hit = container.hitTest(CGPoint(x: 100, y: 100), with: nil)
    XCTAssertNil(hit, "Touches on empty container in toast mode should pass through (return nil)")
  }

  func testPassthroughView_toastMode_passesThroughOnFullScreenWrapperAndBackdrop() {
    let container = IAMPassthroughView(frame: CGRect(x: 0, y: 0, width: 390, height: 844))
    container.isPassthroughEnabled = true

    // SDK adds a full-screen overlay wrapper
    let overlayWrapper = UIView(frame: container.bounds)
    container.addSubview(overlayWrapper)

    // And inside it, a full-screen backdrop view
    let backdrop = UIView(frame: overlayWrapper.bounds)
    overlayWrapper.addSubview(backdrop)

    // And the actual toast card at the bottom
    let toastButton = UIButton(frame: CGRect(x: 20, y: 700, width: 350, height: 60))
    overlayWrapper.addSubview(toastButton)

    // Touch on empty area (hits backdrop or wrapper) must pass through
    let hitEmpty = container.hitTest(CGPoint(x: 200, y: 300), with: nil)
    XCTAssertNil(hitEmpty, "Touches outside toast card must pass through (return nil) even with full-screen wrapper/backdrop")

    // Touch on toast button must NOT pass through (must return the button)
    let hitButton = container.hitTest(CGPoint(x: 100, y: 720), with: nil)
    XCTAssertEqual(hitButton, toastButton, "Touches on toast button must be delivered to the button")
  }

  func testPassthroughView_modalMode_blocksTouches() {
    let container = IAMPassthroughView(frame: CGRect(x: 0, y: 0, width: 390, height: 844))
    container.isPassthroughEnabled = false

    let hit = container.hitTest(CGPoint(x: 100, y: 100), with: nil)
    XCTAssertEqual(hit, container, "Touches on modal IAM background should be intercepted (return container)")
  }

  func testOverlayWindow_nilPassthrough_returnsNil() {
    let window = IAMOverlayWindow(frame: CGRect(x: 0, y: 0, width: 390, height: 844))
    window.passthroughView = nil

    let hit = window.hitTest(CGPoint(x: 100, y: 100), with: nil)
    XCTAssertNil(hit, "When passthroughView is nil, overlay window should return nil")
  }
}
