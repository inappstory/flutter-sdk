...
class FeedStoriesController {
  ...
  Future<void> fetchFeedStories() async {
    ...
    if (_feed?.isEmpty ?? true) {
      if (kDebugMode) {
        print('[InAppStory]: Feed is not set. Please set the feed before calling fetchFeedStories');
      }
      return;
    }
    if (_iasStoryListHostApi == null) {
      // TODO: 08.05.2025 Add Exception
      if (kDebugMode) {
        print('[InAppStory]: Add controller to feed stream before calling fetchFeedStories');
      }
      return;
    }
